class Calculator:
    ...
    def multiply(x,y):
        if not isinstance(x, (int, float)) or not isinstance(y, (int, float)):
            raise TypeError("Les entrées doivent être de type numérique")
        return x * y
    ...
    def square_root(x):
        if not isinstance(x, (int, float)):
            raise TypeError("Les entrées doivent être des entiers positifs")

        if x < 0:
            raise ValueError("Les entrées doivent être des entiers positifs")
